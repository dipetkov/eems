...
mcmc.solution <- function(datapath,dataset,config,dimns) {
    
    filepath <- paste(datapath,'/',dataset,'-',config,sep='')
    print(filepath)
    
    ipmap <- read.table(paste(filepath,'.ipmap',sep=''),header=FALSE)
...
for (simno in simnos) {
    figure <- paste(plotpath,'/',dataset,'-g',xPop,'x',yPop,'-simno',simno,'-soln',sep='')
    config <- paste('simno',simno,'-g',xPop,'x',yPop,sep='')
    mcmc.solution(datapath,dataset,config,dimns)
