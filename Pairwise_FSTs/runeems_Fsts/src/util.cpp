...
bool Params::check_input_params( ) const {
  ...
  if (!(dfmin>=nIndiv) || !(dfmax<=nSites) || !(dfmin<=dfmax)) {
    cerr << "  Error with the DF parameters:" << endl
	 << "  dfmin = " << dfmin << ", dfmax = " << dfmax << endl;
    error = true;
  }
  if (!(nDemes>0) || !(nIndiv>0) || !(nSites>0) || !(nSites>=nIndiv)) {
    cerr << "  Error with the EEMS parameters:   (Is nSites>=nIndiv?)" << endl
...
double pseudologdet(const MatrixXd &A, const int r) {
  SelfAdjointEigenSolver<MatrixXd> x(A);
  return (x.eigenvalues().reverse().array().head(r).log().sum());
}
 ...
 */
MatrixXd pinv(const MatrixXd& m) {
  double epsilon = 1E-9;
...
double pseudowishpdfln(const MatrixXd &X, const MatrixXd &Sigma, const int df) {
  FullPivLU<MatrixXd> luX(X);
  FullPivLU<MatrixXd> luS(Sigma);
  int rX = luX.rank();
  int rS = luS.rank();
  double ldX = pseudologdet(X,rX);
  double ldS = pseudologdet(Sigma,rS);
  int n = X.rows( );
  int q = (df<rS) ? df : rS;
  return (0.5*(-df*ldS - (pinv(Sigma) * X).trace() +
	       (df-n-1.0)*ldX - df*rS*log_2 + df*(q-rS)*log_pi) - mvgammaln(0.5*df,q));
}
...
MatrixXd expected_between_dissimilarities(const MatrixXd &J, const MatrixXd &M) {
  int n = J.rows();
  int o = J.cols();
  MatrixXd Delta = J*resistance_distance(M,o)*J.transpose();
  return (Delta);
