...
one.prob.contour <- function(mcmcpath, dimns, Props, longlat, plot.params, is.mrates,
  ...
  main.title <- paste("Posterior probabilities P(" , v, " > 0 | diffs) and P(", v, " < 0 | diffs)")
  key.greaterthan0 <- paste0("P(", v, " > 0)")
  key.lessthan0 <- paste0("P(", v," < 0)")
  ...
  myfilledContour(rr, col = prob.colors, levels = prob.levels, asp = 1,
                  ...
                  key.title = { mtext(key.greaterthan0, side = 3, cex = 1.5, line = 1.5, font = 1);
...
#' eems_results <- file.path(path.package("rEEMSplots"), "extdata", "EEMS-example")
#' name_figures <- path.expand(file.path("~", "EEMS-example"))
#'
...
#'            longlat = TRUE)
#'
#' datapath <- file.path(path.package("rEEMSplots"), "extdata", "EEMS-example")
...
#'
#' ## Use the provided example or supply the path to your own EEMS run
#' eems_dataset <- file.path(path.package("rEEMSplots"), "extdata", "EEMS-barrier")
#' eems_results <- file.path(path.package("rEEMSplots"), "extdata", "EEMS-barrier")
#' name_figures <- path.expand(file.path("~", "EEMS-barrier"))
#'
...
#' ## Use the provided example or supply the path to your own EEMS run.
#' eems_results <- file.path(path.package("rEEMSplots"), "extdata", "EEMS-example")
#' name_figures <- path.expand(file.path("~", "EEMS-example-grid_is_connected"))
#'
...
#' eems_results <- file.path(path.package("rEEMSplots"), "extdata", "EEMS-popgrid")
#' name_figures <- path.expand(file.path("~", "EEMS-example-grid_is_not_connected"))
#'
#' eems.population.grid(eems_results,
#'                      name_figures,
#'                      longlat = FALSE,
