...
while ~mcmc.finished
  ...
  while ~mcmc.iterDone
    ...
    elseif (mcmc.currType==2)
      %% Propose to move the qVoronoi tiles around %%
      [proposal,pi1_pi0] = ...
...
dlmwrite(strcat(mcmcpath,'/mcmcvS2loc.txt'),mcmcvS2loc,'delimiter',' ','precision','%.6f');
...
dlmcell(strcat(mcmcpath,'/mcmcqrates.txt'),mcmcqRates,'delimiter',' ','precision','%.6f');
...
for s = 1:nSites
  ...
  s0 = repmat(diag(thisSims),1,nIndiv);
  thisDobs = s0 + s0' - 2*thisSims;
  thisPairs = obsrv*obsrv';
  thisDobs(~thisPairs) = 0;
  Dobs = Dobs + thisDobs;
  Pairs = Pairs + thisPairs;
...
for iter = 1:nIters
  ...
  JtDhatJ = JtDhatJ + (B + onev*w'/2 + w*onev'/2);
