...
\begin{itemize}
  ...
  \item \keystring{mcmcpath}: path to the output directory. EEMS creates this directory and saves all results there. There is an accompanying R package which parses the output files and generates several figures to visualize the EEMS results. See \Cref{sec:plotting-eems}.
...
\centering
\includegraphics[height=2.5in]{wang_etal2012-nIndiv314-irregular-habitat-nDemes100-chain0-mrates01.png}
...
\begin{subfigure}[t]{0.47\textwidth}
\centering
\includegraphics[height=2.5in]{wang_etal2012-nIndiv314-irregular-habitat-nDemes400-chain0-mrates01.png}
...
\begin{subfigure}[t]{0.47\textwidth}
\centering
\includegraphics[height=2.5in]{wang_etal2012-nIndiv314-irregular-habitat-nDemes700-chain0-mrates01.png}
...
\begin{subfigure}[t]{0.47\textwidth}
\centering
\includegraphics[height=2.5in]{wang_etal2012-nIndiv314-irregular-habitat-nDemes1000-chain0-mrates01.png}
...
%\subsection{How to load a pre-constructed population grid}

\subsection{How to restart EEMS}

Suppose that \keystring{runeems\_snps} (or \keystring{runeems\_sats}, if the data is microsatellites) has finished running but the MCMC chain has not converged, as indicated by the posterior trace plot. [\Cref{sec:plotting-eems} describes how to generate this plot as well as two other diagnostic figures.]

\begin{figure}[htb]

\begin{subfigure}[t]{0.47\textwidth}
\centering
\includegraphics[height=2.4in]{EEMS-chain-not-converged-pilogl01.png}
\caption{}
\label{fig:EEMS-convergence:not-converged}
\end{subfigure}
\begin{subfigure}[t]{0.47\textwidth}
\centering
\includegraphics[height=2.4in]{EEMS-chain-has-converged-pilogl01.png}
\caption{}
...
\begin{subfigure}[t]{0.47\textwidth}
\centering
\includegraphics[height=2.4in]{EEMS-chain-long-pilogl01.png}
\caption{}
\label{fig:EEMS-convergence:long-chain}
\end{subfigure}
\begin{subfigure}[t]{0.47\textwidth}
\centering
\includegraphics[height=2.4in]{EEMS-chain0_9-pilogl01.png}
\caption{}
...
\subref{fig:EEMS-convergence:not-converged} This MCMC chain obviously has not converged. 
\subref{fig:EEMS-convergence:has-converged} There is no indication that the MCMC chain has not converged. This is not quite the same as there is evidence that the MCMC chain has converged. 
\subref{fig:EEMS-convergence:long-chain} To be confident that EEMS has converged, we can run the MCMC sampler for more iterations.
\subref{fig:EEMS-convergence:multiple-chains} Alternatively, to be confident that EEMS has converged, we can run the MCMC sampler several times, each time starting from a different randomly initialized parameter state.}
...
EEMS aims to model the genetic differentiation between individuals in space. To achieve this, EEMS constructs a triangular grid, assigns each sample to its closest vertex (deme) and estimates a migration rate for every edge and a diversity rate for every vertex in the grid. %\textcolor{red}{Add a figure similar to Figure 1 in the EEMS paper but with diversity parameters as well.}
